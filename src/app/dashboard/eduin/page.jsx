...
export default function EduinDashboard() {
  ...
  // 4. FILTRO DE PERIODO (Por defecto el mes actual de 2026)
  ...
  const fetchDatos = async () => {
    setLoading(true);
    try {
      // Consulta con el operador OR para traer lo propio o lo pagado por mí en la casa
  ...
  // Filtrar movimientos en memoria por el periodo seleccionado y que NO estén anulados
  const filteredTransactions = useMemo(() => {
    return transacciones.filter(tx => 
      tx.fecha_transaccion?.startsWith(selectedPeriod) && tx.estado !== 'anulado'
    );
  }, [transacciones, selectedPeriod]);

  // PROCESAMIENTO MATEMÁTICO DE MÉTRICAS Y GRÁFICOS
  const metrics = useMemo(() => {
    ...
    // 1. DISTRIBUCIÓN POR CATEGORÍAS (Simulación Donut)
    ...
    <div className="w-full max-w-4xl mx-auto p-4 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button onClick={fetchDatos} className="p-2.5 bg-gray-50 rounded-xl border border-gray-200 hover:bg-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-400 uppercase">Mis Ingresos</p>
            <h3 className="text-xl font-bold text-gray-800 mt-1">${metrics.ingresos.toLocaleString('es-CO')}</h3>
          </div>
          <div className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl"><ArrowUpRight className="w-5 h-5" /></div>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-400 uppercase">Mis Gastos + Aportes</p>
            <h3 className="text-xl font-bold text-gray-800 mt-1">${metrics.gastos.toLocaleString('es-CO')}</h3>
          </div>
          <div className="p-2.5 bg-red-50 text-red-600 rounded-xl"><ArrowDownRight className="w-5 h-5" /></div>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-400 uppercase">Saldo Disponible</p>
            <h3 className={`text-xl font-bold mt-1 ${metrics.balance >= 0 ? 'text-indigo-600' : 'text-red-500'}`}>
              ${metrics.balance.toLocaleString('es-CO')}
            </h3>
          </div>
          <div className="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl"><Wallet className="w-5 h-5" /></div>
        </div>
      </div>

      {/* 2. TERMÓMETRO DE CONSUMO */}
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-3">
        <div className="flex justify-between items-center text-sm">
          <span className="font-semibold text-gray-700">Velocidad de Gasto Mensual</span>
          <span className="font-bold text-gray-600">{metrics.porcentajeConsumo.toFixed(1)}% del Ingreso</span>
        </div>
        <div className="w-full bg-gray-100 h-3 rounded-full overflow-hidden">
          <div 
            className={`h-full transition-all duration-500 ${metrics.porcentajeConsumo >= 80 ? 'bg-red-500' : metrics.porcentajeConsumo >= 50 ? 'bg-amber-500' : 'bg-emerald-500'}`}
            style={{ width: `${Math.min(metrics.porcentajeConsumo, 100)}%` }}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-indigo-50 to-white p-4 rounded-2xl border border-indigo-100/50">
          <span className="text-xs font-medium text-indigo-600 uppercase tracking-wider">Capacidad de Ahorro</span>
      ...
      {/* 1. DISTRIBUCIÓN EN BARRAS NATURAS (DONUT ALTERNATIVE) */}
      ...
      <TransactionForm isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} defaultScope="eduin" onActionSuccess={fetchDatos} />
    </div>
  );
}
