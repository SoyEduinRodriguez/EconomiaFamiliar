...
export default function MajoDashboard() {
  ...
  const fetchDatos = async () => {
    setLoading(true);
    try {
      // Trae los movimientos personales de Majo + lo que Majo pagó del Hogar en este mes específico
      ...
      console.error('Error cargando finanzas de Majo:', err.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-1.5 px-4 py-2 bg-rose-500 text-white rounded-xl font-bold text-xs shadow-sm hover:bg-rose-600">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-rose-500 to-slate-950 p-4 rounded-2xl text-white shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-rose-200 uppercase">Disponible / Ahorro</p>
            ...
            {metrics.distribucion.map(cat => (
              <div key={cat.name} className="space-y-1">
                ...
                <div className="w-full bg-gray-50 h-2 rounded-full overflow-hidden">
                  <div className="h-full bg-rose-500" style={{ width: `${cat.porcentaje}%` }} />
                </div>
              </div>
            ))}
          </div>
